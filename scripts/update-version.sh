...
sed -E -i "" "s/(VERSION_NUMBER[[:blank:]]*=[[:blank:]]*).*;/\1$NORMALISED_VERSION;/" ./Shepherd.xcodeproj/project.pbxproj
echo "Setting version in Podspec to $NORMALISED_VERSION"
sed -E -i "" "s/(spec\\.version[[:blank:]]*=[[:blank:]]*)\".*\"/\1\"$NORMALISED_VERSION\"/" ./Shepherd.podspec
